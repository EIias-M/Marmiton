...
<body>
	...
	<h1>Liste de tous les Ustensiles</h1>

	<?php

        // affichage de la liste des tâches
		foreach ($lesUst as $Usten) {
			echo " <a href='routeurUstensile.php?action=AfficherUnUstensile&NomUstensile={$Usten->getnomUstensile()}'> <p>Ingredient {$Usten->getnomUstensile()} </p></a>";
			if(!empty($_SESSION['Admin'])){
				if($_SESSION['Admin']=='2'){
		 			echo "<a href='routeurUstensile.php?action=delete&NomUstensile={$Usten->getnomUstensile()}'> supprimer l'ingredient </a> | <a href='routeurUstensile.php?action=update&NomUstensile={$Usten->getnomUstensile()}'> modifier l'ingredient </a></p>";
				}
			}		
		}
		echo "<br><br>";
		if(!empty($_SESSION['Admin'])){
			if($_SESSION['Admin']=='2'){
				echo "<a href='routeurUstensile.php?action=create'> créer un Ustensile </a>";
			}
		}
